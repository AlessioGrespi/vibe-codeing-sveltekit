import { google, github } from '$lib/auth/oauth';
...
export const GET: RequestHandler = async ({ url, cookies, fetch }) => {
  ...
  let provider: string;
  if (path.includes('/oauth/callback/google')) {
    provider = 'google';
  } else if (path.includes('/oauth/callback/github')) {
    provider = 'github';
  } else {
    throw redirect(303, '/login?error=oauth_provider');
  }
  ...
  if (provider === 'google') cookies.delete('oauth_code_verifier', { path: '/' });
